angular.module("application").service('Person', ["$resource", "$modal", function ($resource, $modal) {
    return $resource("/odata/Person(:id)", { id: "@id" }, {
        ...
        "GetCurrentUser" : {
            url: "/odata/Person/Service.GetCurrentUser?$select=Id,IsSubstitute,RecieveMail,IsAdmin,FullName,Mail,DistanceFromHomeToBorder &$expand=PersonalRoutes($expand=Points),LicensePlates,Employments($select=Id,HomeWorkDistance,Position,WorkDistanceOverride,AlternativeWorkAddressId;$expand=AlternativeWorkAddress;$expand=OrgUnit($select=Id,LongDescription,HasAccessToFourKmRule;$expand=Address))",
            method: "GET",
